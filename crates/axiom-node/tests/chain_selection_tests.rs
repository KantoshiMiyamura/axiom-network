...
fn test_local_mined_then_better_remote_chain() {
    ...
    assert_eq!(
        height_after_local, 1,
        "After mining locally, height should be 1"
    );
    ...
    assert_eq!(
        next_block.header.prev_block_hash, local_hash,
        "Next block should reference our local block"
    );
}
...
fn test_invalid_remote_block_rejected() {
    ...
    assert_eq!(
        tip_after, initial_tip,
        "Tip should not change after invalid block"
    );
    assert_eq!(
        height_after, initial_height,
        "Height should not change after invalid block"
    );
}
...
fn test_mining_snapshot_staleness() {
    ...
    let snapshot1 = axiom_node::MiningSnapshot::new(tip1, height1, 0x207fffff, vec![], 0, 1);

    // Verify snapshot is not stale
    assert!(
        !snapshot1.is_stale(&tip1, 1),
        "Snapshot should not be stale with same tip and version"
    );

    // Simulate new best tip
    let tip2 = Hash256::from_bytes([1u8; 32]);
    assert!(
        snapshot1.is_stale(&tip2, 1),
        "Snapshot should be stale with different tip"
    );

    // Simulate version change
    assert!(
        snapshot1.is_stale(&tip1, 2),
        "Snapshot should be stale with different version"
    );
}
...
fn test_block_index_fork_tracking() {
    ...
    index.insert(
        ...
        0x207fffff,
        1,
        axiom_node::BlockSource::LocalMined,
    );
    ...
    assert_eq!(
        ...
        2,
        "Genesis should have 2 children (fork)"
    );
    assert!(
        genesis_children.contains(&block_a),
        "Genesis children should include A"
    );
    assert!(
        genesis_children.contains(&block_b),
        "Genesis children should include B"
    );
    ...
    assert_eq!(
        best_at_1,
        Some(block_b),
        "Block B should be best at height 1 (higher chainwork)"
    );
}
...
fn test_height_calculation_correctness() {
    ...
    let _genesis_hash = node.best_block_hash().unwrap();
...
fn test_chainwork_accumulation() {
    ...
    assert!(
        work_1 > work_genesis,
        "Chainwork should increase after block 1"
    );
...
fn test_no_height_based_chain_selection() {
    ...
    let _tip1 = node.best_block_hash().unwrap();
    let height1 = node.best_height().unwrap();
    let _work1 = node.get_chain_work().unwrap().unwrap();
