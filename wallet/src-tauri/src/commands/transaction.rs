...
pub async fn prepare_send(
    ...
    if amount_sat < DUST {
        return Err(AppError::DustAmount);
    }
    ...
    let needed = amount_sat
        .checked_add(fee)
        .ok_or(AppError::Internal("overflow".into()))?;
    ...
    *state
        .pending_tx
        .lock()
        .map_err(|_| AppError::Internal("lock".into()))? = Some(PendingTx {
        to_address: to.clone(),
        amount_sat,
        fee_sat: fee,
...
pub async fn confirm_send(state: tauri::State<'_, AppState>) -> AppResult<SendResult> {
    ...
    let signed_hex = {
        ...
        let mut b = TransactionBuilder::new()
            ...
            .nonce(ptx.nonce);
        for u in &ptx.utxos {
            let txid = Hash256::from_slice(
                &hex::decode(&u.txid).map_err(|_| AppError::Internal("hex".into()))?,
            )
            .map_err(|e| AppError::Internal(e.to_string()))?;
            b = b.add_input(txid, u.output_index);
        }

        let to = Address::from_string(&ptx.to_address)
            .map_err(|_| AppError::InvalidAddress(ptx.to_address.clone()))?;
        b = b.add_output(
            Amount::from_sat(ptx.amount_sat).map_err(|e| AppError::Wallet(e.to_string()))?,
            *to.pubkey_hash(),
        );
        ...
        if change >= DUST {
            let from = Address::from_string(&ptx.from_address)
                .map_err(|_| AppError::InvalidAddress(ptx.from_address.clone()))?;
            b = b.add_output(
                Amount::from_sat(change).map_err(|e| AppError::Wallet(e.to_string()))?,
                *from.pubkey_hash(),
            );
        }
    ...
    match rpc.tx_history(&addr).await {
        Ok(txs) => {
            state
                ...
                .lock()
                .map_err(|_| AppError::Internal("lock".into()))?
                .set_transactions(&addr, txs.clone());
        ...
        Err(_) => {
            ...
            Ok(c.get_txs(&addr)
                .iter()
                .map(|t| TxHistoryEntry {
                    txid: t.txid.clone(),
                    block_height: t.block_height,
                    timestamp: t.timestamp,
                    value_change: t.value_change,
                    direction: if t.value_change >= 0 {
                        "received"
                    } else {
                        "sent"
                    }
